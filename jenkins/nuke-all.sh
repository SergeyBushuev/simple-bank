...
for ns in simple-bank-dev simple-bank-test simple-bank-prod; do
  helm uninstall simple-bank -n "$ns" || true
done
...
kubectl delete ns simple-bank-dev --ignore-not-found
kubectl delete ns simple-bank-test --ignore-not-found
kubectl delete ns simple-bank-prod --ignore-not-found
